...
public class Action {
  ...
  public static Action makePickup(Order order) {
    return new Action(ActionType.PICKUP, order.getId(), null);
  }

  public static Action makeDropoff(Order order, LocalDateTime deliveryTime) {
    return new Action(ActionType.DROPOFF, order.getId(), deliveryTime);
  }

  public static Action makePickupForOrder(long orderId) {
