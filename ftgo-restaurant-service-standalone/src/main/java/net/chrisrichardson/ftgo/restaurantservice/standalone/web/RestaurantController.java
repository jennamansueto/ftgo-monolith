...
import net.chrisrichardson.ftgo.restaurantservice.standalone.domain.Restaurant;
  ...
  private GetRestaurantResponse makeGetRestaurantResponse(Restaurant r) {
    return new GetRestaurantResponse(r.getId(), r.getName(),
            r.getMenuItems().stream()
                    .map(mi -> new GetRestaurantResponse.MenuItem(mi.getId(), mi.getName(), mi.getPrice()))
