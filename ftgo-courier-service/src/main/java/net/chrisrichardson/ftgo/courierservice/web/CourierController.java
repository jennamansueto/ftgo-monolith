...
public class CourierController {
  ...
  @RequestMapping(path="/couriers/assign-delivery", method= RequestMethod.POST)
  public ResponseEntity<AssignDeliveryResponse> assignDelivery(@RequestBody AssignDeliveryRequest request) {
    try {
      Courier courier = courierService.assignDelivery(request.getOrderId(), request.getPickupTime(), request.getDropoffTime());
      return new ResponseEntity<>(new AssignDeliveryResponse(courier.getId()), HttpStatus.OK);
